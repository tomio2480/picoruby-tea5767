...
ブラウザ側（Phase 2 / 3）と firmware 側（Phase 4）のコードが揃った．
本ノートでは Phase 5（函館実測）に向けた実機接続手順を整理する．
PicoRuby R2P2 はあらかじめ Pico に書き込み済みである前提（本リポジトリ対象外）．
...
- `require_relative` が動かない場合は， `firmware/lib/*.rb` の内容を `main.rb` の先頭に結合する．
  結合した 1 ファイルをアップロードして回避する．
- `I2C.new(unit:, frequency:)` / `sleep_ms(ms)` / `$stdout.puts` の API は R2P2 のバージョン依存．
  実装と異なる場合は `main.rb` 側で調整する．
...
| 受信中のまま描画や結果が更新されない | firmware 側の JSON 出力が壊れていて `Protocol.parse` が `nil` を返す | `firmware/main.rb` に `$stdout.flush` を入れる．`SerialEmitter` の JSON 生成が正しいか `firmware/spec` で再確認． |
