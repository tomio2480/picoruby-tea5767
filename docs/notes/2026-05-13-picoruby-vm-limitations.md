...
R2P2 のバージョンは 3.4.2 の PICO ビルド（`R2P2-PICORUBY-3.4.2-PICO-20260329-9b94521d.uf2`）である．
...
`SpectrumScanner.new(receiver, start_hz: ..., step_hz: ...)` のように，
**位置引数を 1 つ以上 + キーワード引数を複数** 渡すと VM 整合が崩れる．
`Oops, return value is gone` が出て戻り値が消失し，代入先が前の束縛のまま残る．

`I2C.new(unit: ..., sda_pin: ..., ...)` のような **キーワード引数のみ** ，
`emitter.tick(i: ..., f: ...)` のような全部キーワードのケースは問題なく動く．

### 3. ARYSPLAT（opcode 0x56）が未実装

`Unimplemented opcode (0x56) found (Exception)` の 0x56 は，
MRuby の `include/mruby/ops.h` で 86 番目にあたる `ARYSPLAT`（配列スプラット展開命令）である．
コードに明示的な `*` がなくても，「位置 + キーワード混在呼び出し」や「ラムダの動的生成」のコード生成時に内部で ARYSPLAT を含むバイトコードが出る．
...
`picoruby.org` の手順では UF2 投入後に `R2P2` という名前のドライブが見えるはずだが，
3.4.2 PICO ビルドでは MSC 機能しなかった．`flash_nuke.uf2` で完全クリアして再インストールしても結果は同じ．
...
- LED フィードバック（GPIO は IRB で動作確認済み）の app.rb への組み込みは保留．rescue を使わずに「初期化完了で点灯」を表す書き方を検討する．
- 将来 PicoRuby が mruby から強化された先で VM 不整合が解消される可能性がある．3.4.2 が mruby/c VM の最終版で，以降は mruby VM に移行する．移行版で再評価する価値がある．
- 同じ「位置 + キーワード混在」「ラムダ動的生成」を PicoRuby の Issue として報告するかを検討する．
