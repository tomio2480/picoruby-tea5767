...
`_tmp_` プレフィックスを持つファイルやディレクトリは， `.gitignore` の `/_tmp_*` パターンによって一括で除外される．
コピーした `prh.yml` は `trap ... EXIT` により Ctrl+C 中断時もスクリプト終了時に必ず削除される．本リポジトリには直接コミットしない．
