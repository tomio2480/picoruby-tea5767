...
ボタンの有効条件は「Pico 接続中かつ選局済みかつ停止していない」に絞った．
...
Pico との接続が切断されたとき，`on_stream_end` が処理していたのは
`pico_client = nil` と `scan_pico_btn[:disabled] = true` のみだった．
`is_scanning = false` のリセットが漏れていた．

結果として，切断後も `is_scanning = true` が残り続けた．
以後の `canvas.click` は `next if is_scanning` で早期 return し続けた．
...
スキャン開始ハンドラと `on_stream_end` に `stop_btn[:disabled] = true` を追加した．
対象ハンドラは `start_mock_btn.click` / `scan_pico_btn.click` の 2 種．
