...
Phase 1 として `web/` 下の純ロジック（Aggregator 等 4 本）を TDD で実装した．結果は 54 tests / 69 assertions 緑．PR #3 対応（Aggregator drop 追跡・初期化ガード，Protocol 型検証，StationDirectory Hz 比較）を含む．この過程で得た技術判断と運用知見を記録する．
...
当初 README スケルトンには `@rssi[ch_start...ch_end].max || 0` と書かれていた．しかし直前で `ch_end = ch_start + 1 if ch_end <= ch_start` のガードがあり，スライスは常に 1 要素以上になる．`|| 0` は実際には発動しない防御コードで，Defensive Programming のコストが可読性を下回ると判断して削除した．
...
ステートレスな純関数は，意図を伝えるために `class` ではなく `module module_function` を選択した．`PeakDetector.pick_max` などの内部ヘルパは `private_module_function` にできるが，KISS で公開のまま．将来テストから直接呼びたくなる可能性も含めて開けておく方が手間を増やさない．
...
- `stations.json` が肥大化する将来に備え，smoke テストの追加タイミングを検討する．実 JSON を読ませて `StationDirectory` が破綻しないかを確認するテストが該当する．
