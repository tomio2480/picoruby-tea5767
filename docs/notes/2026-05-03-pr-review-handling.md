...
5. **返信投稿** ． `gh api .../pulls/{N}/comments` に `-f body` と `-F in_reply_to` を付けて POST する．まとめて投稿する場合はループで実行する．
...
`--merge` オプションのみでマージし，remote ブランチを後から手動削除すると **PR が CLOSED 化** する．
GitHub の自動切替（base を `main` に変更）は，merge と branch delete を同一 API で連続実行した場合のみ発動する．
...
Gemini が `**手段** （` の半角スペース挿入を採用したところ，reviewdog が逆に「括弧外側のスペースを外せ」と新規指摘した．CLAUDE.md / styleguide.md と Gemini が一致して挿入を要求しているため，textlint 指摘を reject で揃えた．
