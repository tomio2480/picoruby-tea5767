...
レビュー対応は次の 5 段で進める．各段で取りこぼしが起きやすい箇所が異なるので， 1 段ずつ完了を確認してから次へ進む．

1. **paginate 取得** ． `gh api .../pulls/{N}/comments --paginate` で全 review comment を取得する．reviews と issues/comments も同様に paginate 付きで取る．
2. **未返信抽出** ．自分（PR 著者）の reply 先 ID 集合を作り，root コメント（ `in_reply_to_id == null` ）のうち集合に含まれない ID を未返信とする．
...
5. **返信投稿** ． `gh api .../comments/{ID}/replies` で個別投稿する．まとめて投稿する場合はループで実行する．
...
これは textlint の `ja-spacing/ja-no-space-around-parentheses` （括弧の外側にスペースを入れない）と直接衝突する．Gemini の references でも `This takes precedence over general textlint suggestions to remove spaces before punctuation` と明記されており， **強調マーカー規律が上位** で textlint 指摘は reject する．
...
`gh pr merge --merge` （delete-branch なし）でマージし， **後から remote ブランチを手動で削除** すると，スタック先 PR の base が GitHub の自動切替（base を `main` に変更）対象にならず， **PR が CLOSED 化** してしまう．自動切替は merge と branch delete が同じ API 呼び出しで連続実行された場合のみ発動する．
...
`gh api .../pulls/5/comments --jq '...'` を paginate なしで実行し， 30 件以降の Gemini HIGH 重要度指摘 3 件を「存在しない」と判定した．サブエージェントが正しく拾った ID を「捏造」と誤判定する事態にまで至った．paginate 付きで再確認したところ，さらに 3 件（ PR #6 を含む計 6 件）の未返信が見つかった．
...
Gemini が `**手段** （` のように半角スペース挿入を要求し，採用したところ， reviewdog（textlint）が逆に「括弧外側のスペースを外せ」と新規指摘してきた．Gemini と CLAUDE.md / styleguide.md が一致して挿入を要求している側であり，textlint 指摘を reject で揃えた．

### 3.4 PR #7 の CLOSED 化

PR #5 を `gh pr merge --merge` （delete-branch なし）でマージし，後から `git push origin --delete feat/phase4-firmware` で削除した．これにより PR #7 のベースが自動切替されず CLOSED された．上記 [2.4](#24-stacked-pr-の親をマージするときは---delete-branch-必須) の手順で復旧した．

## 4. 参照

- 本リポジトリ Claude memory（git 管理外） ．paginate 規律 / 強調マーカー規律 / stacked PR 規律の 3 件は `feedback_gh_api_paginate.md` ・ `feedback_emphasis_vs_textlint_paren.md` ・ `feedback_stacked_pr_branch_delete.md` に記録済み
