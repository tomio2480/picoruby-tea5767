...
これにより `PicoRuby` 固有の `sleep_ms` への直接依存が純ロジックから消え，CRuby 上で TDD が成立する．`canvas_renderer` / `mock_stream`（web 側）でも同じパターンを採った．**「JavaScript / PicoRuby 依存は境界で注入する」** が本プロジェクト全体の方針として定まった．
