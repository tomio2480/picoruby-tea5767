...
- 後続 4 段階（Phase 1〜4）と最終 cleanup を 5 本の PR に切り分ける．web トラック 3 本，firmware トラック 2 本の 2 系統でスタック並走させる．
...
- 現状 `firmware/lib/tea5767.rb` の `pll_for` は整数除算（floor 等価）で 9304 を返す．76.0 MHz では誤差 −6.6 kHz となり，基準を満たさない．
...
- merge 後，本リポジトリに `.textlint-allowlist.yml` を追加する（[github-workflows#14](https://github.com/tomio2480/github-workflows/issues/14) 実装待ち）．
