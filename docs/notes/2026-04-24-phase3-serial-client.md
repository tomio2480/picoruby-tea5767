...
Web Serial API では Promise を返す API が 4 つある．
具体的には `requestPort` / `port.open` / `getReader` / `reader.read` のすべて．
全段でこのパターンを使う．
...
| JavaScript 側で `ReadableStream.pipeThrough(TextDecoderStream).pipeThrough(自作 LineSplitStream)` | 「両端 Ruby」訴求を損なう．行分割は Ruby で十分軽い． |
