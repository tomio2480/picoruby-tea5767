...
![I2C（SDA・SCL・3V3・GND）で Pico と TEA5767 を接続した配線図．ANT に 75 cm ワイヤ，USB を PC に接続する．](./schematic.svg)
...
| `firmware/app.rb` | `$stdin.gets` でコマンド待ち．"SCAN" 受信時のみ 1 周スキャンを実行し JSON Lines を出力する．R2P2 では `/home/app.rb` が自動起動ファイル |
...
| `web/lib/serial_client.rb` | Web Serial の接続・JSON Lines の行分割・Pico へのコマンド送信（`write(text)`） | Ruby.wasm |
...
純ロジック 5 本を CRuby でそのまま minitest にかけられる．
これにより Pico や実ブラウザがなくても大半のバグを潰せる．
<!-- textlint-disable ja-technical-writing/sentence-length -->
対象は `protocol.rb` / `aggregator.rb` / `peak_detector.rb` / `station_directory.rb` / `mock_source.rb` の 5 ファイル．
...
同調周波数は PLL 分周比 N（整数）として表現される．
N は離散値であるため目標周波数をぴったり再現できるとは限らない．
そこで N の選定基準を **「実周波数と目標周波数の誤差を最小にする整数」** と定める．
N が 1 増えるごとに実周波数は f_ref / 4 = 8.192 kHz だけ線形に変化する．
このため連続解の最近整数を選べば誤差最小が成立し，
これは数学的に四捨五入（round）と等価である．
高側ヘテロダインを選んだ場合の計算式は次のとおり．

```
N = round(4 × (f_RF + f_IF) / f_ref)
  f_IF  = 225 kHz
  f_ref = 32.768 kHz
  選定基準：実周波数と目標周波数の誤差を最小にする整数
  線形関係下では「最近整数 = round」と等価
```

具体例は次のとおり．

- 82.5 MHz：連続解 10098.27，N = 10098（floor / round いずれでも同値）．
- 76.0 MHz：連続解 9304.81，N = 9305．
...
| ブロック | 役割 |
|---|---|
| 地域セレクタ | 局プリセットの地域を選択（函館市・旭川市・札幌市・東京 23 区） |
| 「モックスキャン開始」ボタン | MockStream で擬似 tick を流して UI 全体を動作確認．Pico 非接続でも動く |
| 「Pico に接続」ボタン | Web Serial の `requestPort()` を起動して実機と接続．成功後は「スキャン実行」ボタンが有効化される |
| 「スキャン実行」ボタン | 「Pico に接続」成功後に有効化．押下のたびに 1 周スキャンを実行し，完了後に再び有効化される |
...
視認性は十分に取れる．
OLED 時代に必要だった「複数 ch を 1 px に集約する処理」はデフォルト表示では不要になる．
横幅が 191 px を下回る端末向けには `aggregator.rb` を残しておく．

![ブラウザ UI のレイアウト概念図．上部に地域セレクタと接続ボタン，中央に Canvas のスペクトラムバーグラフ，下部に検出局テーブルを配置する．](./display_layout.svg)
...
class TEA5767
  ...
  def self.pll_for(freq_hz)
    # 選定基準：実周波数と目標周波数の誤差を最小にする整数 N を返す．
    # 線形関係下では連続解の最近整数（= round）と等価．
...
他のファイルは Phase 1〜4 の TDD で実装済みである．

## 🔁 通信プロトコル

Pico とブラウザは USB CDC シリアル上で双方向に通信する．
ボーレートは 115200 bps を基本とする．

### ブラウザ → Pico（コマンド）

ブラウザはテキスト行を Pico に送信する．

| コマンド | 意味 |
|---|---|
| `SCAN\n` | 1 周スキャンの開始を要求する |

Pico 側は `$stdin.gets` で 1 行受け取り，`.strip == "SCAN"` を判定してスキャンを開始する．

### Pico → ブラウザ（JSON Lines）
...
HBC / STV のワイド FM は 2026-04-23 時点で函館での運用を確認できないため，プリセットには含めない．
...
Raspberry Pi Pico への PicoRuby 書き込み手順は R2P2 公式ドキュメントに従う．
概略は BOOTSEL 起動中に UF2 ファイルをドラッグ＆ドロップする手順である．
本リポジトリの対象外とする．

### firmware の転送

本リポジトリの `firmware/app.rb` と `firmware/lib/*.rb` を R2P2 のファイルシステムへ転送する．

- `firmware/app.rb` を `/home/app.rb` に配置する．R2P2 は起動時に **`/home/app.rb` を自動実行する** ．
- `firmware/lib/*.rb` を `/home/lib/` 配下に配置する．`app.rb` は **絶対パスで読み込む** 必要がある．PicoRuby には `require_relative` がないため，`require "/home/lib/tea5767"` のように書く．
- 転送方法は R2P2 のバージョンに依存するため，R2P2 のドキュメント（R2P2 シェルの `cp` コマンド，DFU 経由のアップロード等）に従う．
- 確定済み API の詳細は [`docs/notes/2026-04-24-picoruby-api-confirmed.md`](../docs/notes/2026-04-24-picoruby-api-confirmed.md) を参照する．
...
モックデータでブラウザ描画を確認し，
最後に実機（Pico）接続へ降りていく段取りとする．

表 10. マイルストーン．

| 段階 | 内容 | 完了条件 | 状態 |
|---|---|---|---|
| Phase 1 | web 純ロジックの TDD | `aggregator` / `protocol` / `peak_detector` / `station_directory` / `mock_source` の minitest が緑（70 runs） | 完了 |
| Phase 2 | ブラウザ描画＋モックソース | `mock_source` で擬似 tick を流して Canvas にバーグラフと局名が描ける | 完了 |
| Phase 3 | Web Serial 接続 | 実 Pico または USB CDC 相当装置から受けた JSON Lines で描画できる | 完了 |
| Phase 4 | firmware 実装 | Pico 実機で 191 ch スイープと JSON Lines 出力が成立（24 runs） | 完了 |
| Phase 5 | 函館での実測 | RubyKaigi 会場で既知局と可視化結果が一致 | 未着手 |
...
PicoRuby と Ruby.wasm はいずれも CRuby 互換サブセットである．
I/O 境界を抽象化すればロジック部分は CRuby でテスト可能になる．
...
本プロジェクトは受信のみを行う．
TEA5767 は受信 IC であり送信機能を持たないため，電波法上の免許や微弱電波規制の対象にはならない．
受信専用機器の免許不要は電波法第 4 条および第 29 条にもとづく．
根拠の確認は [総務省 電波利用ホームページ](https://www.tele.soumu.go.jp/j/ref/material/rule/) を参照．
アンテナを屋外に延長する場合のみ避雷対策を別途検討する．

将来 NS73M や NJM2035 を送信側として組み合わせる場合は，
微弱電波の基準（3 m で 500 μV/m 以下）に収めること．
あるいは適合表示のある完成品を使う前提で別途計画を立てる．
<!-- textlint-disable -->
この基準は電波法施行規則第 6 条第 1 項第 1 号（微弱無線局）にもとづく．
<!-- textlint-enable -->

> [!NOTE]
> 本セクションの記述は法的助言ではない．実装時は総務省または専門家への確認が必要．

v0.1 は受信のみのため上記後半（送信側）は実装対象外だが，
本実装計画を順当に進めると v0.2 以降で送信側の実験が視野に入る．
そのタイミングで改めて参照できるよう，ここに書き残しておく．

## 📚 参考資料

- PicoRuby 公式ドキュメント：https://picoruby.github.io/
- PicoRuby I2C API（picoruby/picoruby 本体）：https://github.com/picoruby/picoruby/blob/master/mrbgems/picoruby-i2c/mrblib/i2c.rb
