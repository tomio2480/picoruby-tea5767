...
PicoRuby を載せた Raspberry Pi Pico と TEA5767 FM 受信モジュールで
...
市販ラジオでは局の強弱がわかりにくいので，
RSSI（電波強度指標）をブラウザのバーグラフで描くことで，
どの帯域が活発でどこが空いているかを一目で判別できるようにする．

本プロジェクトにはもう一つの目的として，
PicoRuby（組み込み側）と Ruby.wasm（ブラウザ側）の両端を
Ruby で実装することを置く．
RubyKaigi での披露を念頭に，電波を受け取る Pico 側と，
それを可視化するブラウザ側のいずれも Ruby で書き切ることで，
Ruby がマイコンとブラウザの両端でつながる題材として成立させる．
...
`fm_spectrum_monitor.kicad_sch` が本体で，
`fm_spectrum_monitor.kicad_pro` とセットで
KiCad 8 以降で開くことができる．
...
| web | `web/` | Web Serial で JSON Lines を受け取り，集約・ピーク検出・局名付与を行って Canvas に描画 | PC ブラウザ（Ruby.wasm + js） |
...
純ロジックの 4 本（`protocol.rb` / `aggregator.rb` / `peak_detector.rb` / `station_directory.rb`）は
CRuby でそのまま minitest が走る．
これにより Pico も実ブラウザも不要な状態で大半のバグを潰せる．
